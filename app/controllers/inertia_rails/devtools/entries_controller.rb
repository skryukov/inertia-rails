...
module InertiaRails
  module Devtools
    # The read API the extension polls: recorded entries, newest first.
    class EntriesController < ActionController::Base
      protect_from_forgery with: :null_session

      # The extension fetches an entry as soon as the response headers arrive,
      # and the entry is written when the body closes; it takes a 404 as final,
      # so a miss is read again a few times first.
      GRACE_READS = 3
      GRACE_WAIT = 0.05
      ...
      def find_entry(id)
        repository = InertiaRails::Devtools.repository
        entry = repository.get(id)
        return entry if entry

        GRACE_READS.times do
          sleep GRACE_WAIT
          entry = repository.get(id)
          return entry if entry
        end
