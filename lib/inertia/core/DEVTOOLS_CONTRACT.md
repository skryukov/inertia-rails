...
Ruby implementation in `lib/inertia/core/devtools/` and
...
NEXT PR (prop classification, redaction and the source locators land in the
PR after this one; the verdict written for them is the one to re-check then).
...
NEXT PR: today a row carries `shared`, `inertiaType: null`, and the `reset` /
`rescued` verdicts the ledger gave; the classifier that fills the badge in
(`inertiaType`, `deferGroup`, `once`, `mergeDirection`, `deepMerge`, `live`)
is the next PR's.

| # | Contract (quoted) | Verdict |
|---|---|---|
| D1 | `inertiaType` wire values: `always, defer, optional, merge, scroll, once`; plain value -> `null` (L499-507, L1959-1970) | NEXT PR — MATCH, + `live: true` flag for live props (`inertiaType` stays null) |
| D2 | DeferProp reads as `defer` + `deferGroup` ONLY under the deferred header; on a manual partial reload `inertiaType` is null and no `deferGroup` key (L1888-1891, L1963, test L3463-3496) | NEXT PR — MATCH |
| D3 | Other deferrables (`scroll->defer('g')`) carry `deferGroup` regardless of the header; default group `'default'` (L1912-1923, tests L5457-5482) | NEXT PR — MATCH (`Announcements::Defer::DEFAULT_GROUP = 'default'`; a scroll prop's group is nil unless `defer:`) |
| D4 | `once` = `Onceable && shouldResolveOnce()` independent of type (L1897, test L5484-5502) | NEXT PR — MATCH (read from `onceProps` in the page metadata rather than the prop; same answer) |
| D5 | `mergeDirection`: `'prepend'` when `prependsAtRoot()` or (nested prepend && no nested append); else `'append'`; null when not mergeable. Scroll default `append`. Mixed nested reads `append`. Direction is read from the prop, "so it survives deep merges" and a reset (L1939-1953, tests L5504-5558) | NEXT PR — DIVERGES, deliberate: direction is read from the page metadata, so a reset prop carries no `mergeDirection` (Rails spec "flags a reset prop and drops its merge direction" pins it). `deep_merge` + `prepend` is refused by the Ruby prop model, so that sub-case is N/A |
| D6 | `deepMerge` = `shouldDeepMerge() \|\| count(matchesOn()) > 0` (L1929-1932) | NEXT PR — MATCH |
| D7 | `reset` = path in `X-Inertia-Reset` comma list; empty header never resets (L1896, L1975-1978) | MATCH (via `outcome.reset?`) |
| D8 | Per-prop entry keys: `shared` (bool, always), `inertiaType` (always, nullable), then only-when-set: `deferGroup, shareSource, reset, once, mergeDirection, deepMerge, rescued, renderSource` (L177-208, L272-275) | NEXT PR — MATCH, + `live` |
| D9 | `shared` = top-level key is in the shared props (first dotted segment) (L178, L2491-2502) | MATCH |
| D10 | Prune: dotted paths dropped unless `shared === true \|\| inertiaType !== null \|\| count(meta) > 2`; every top-level path kept (L287-302) | MATCH (a nested row survives on its `reset` / `rescued` verdict; the badge cases are NEXT PR) |
...
| D13 | Rescued deferred prop: `inertiaType: 'defer'`, `rescued: true`, absent from `propValues` (L2378-2381, test L3498-3519) | MATCH for `rescued: true` and the absent value; NEXT PR — `inertiaType: 'defer'` |
| D14 | Array element index paths are not recorded as separate props unless badged (`tags.0` absent) (test L3394-3397) | NEXT PR — MATCH (`rows.1.tag` is kept when the element carries an `always`) |

## E. Redaction

NEXT PR: nothing is redacted or sanitized yet; every verdict below is the next
PR's to earn. Until then a value JSON refuses costs the whole entry (reported).

| # | Contract (quoted) | Verdict |
|---|---|---|
| E1 | Default keys: `password, password_confirmation, current_password, token, _token, access_token, refresh_token, secret, client_secret, api_key` (L57-68) | NEXT PR — MATCH (`devtools_redact_keys`), + the app's `filter_parameters` |
| E2 | Default headers: `cookie, set-cookie, authorization, proxy-authorization, x-xsrf-token, x-csrf-token` (L70-77) | NEXT PR — MATCH (`devtools_redact_headers`) |
| E3 | Markers `'[REDACTED]'` and `'[UNSERIALIZABLE]'` (L1995-1997) | NEXT PR — MATCH |
| E4 | Key redaction is case-insensitive, recursive, replaces the whole subtree (L2039-2050, test L5720-5735) | NEXT PR — MATCH |
| E5 | Headers flattened to strings, multi-values joined `', '`, sensitive names replaced (L2056-2069, test L4544-4562) | NEXT PR — MATCH, + URL-bearing headers (`location`, `referer`, `x-inertia-location`, ...) get their query redacted |
| E6 | URL redaction on `url` and `redirectLocation` keys: sensitive query params (incl. nested `filter[secret]`) -> `[REDACTED]`; unparseable returned unchanged (L2095-2171, tests L5737-5767) | NEXT PR — DIVERGES, deliberate: an unparseable query is dropped to `?[REDACTED]` instead of stored raw (fail closed; both suites pin it) |
| E7 | Storage pass = redact keys over the whole payload + redact urls + redact header bags + sanitize (L2023-2032) | NEXT PR — DIVERGES, deliberate: each surface is redacted once when captured; the storage pass only redacts URLs and sanitizes. Running the key filter over the whole entry, as Laravel does, would replace the metadata row of a prop *named* `password` with the marker; the Rails spec "keeps metadata for props named like sensitive keys" pins the Ruby choice |
| E8 | `propValues` redacted with the key list and sanitized at build time (L1409-1413) | NEXT PR — MATCH |
...
| F3 | requestBody: JSON body decoded + redacted; empty -> `empty` (L1566-1570) | MATCH for the decoding and `empty`; the redaction is NEXT PR |
| F4 | requestBody: form input redacted, uploads summarized `{name, size (null if invalid), mimeType}` (L1572-1576, L1751-1774) | MATCH for the upload summary (size is always reported); the redaction is NEXT PR |
...
| F6 | responseBody for an Inertia render: page object JSON-normalized then redacted, `status: present` (L1598-1634) | MATCH for the normalized page; the redaction is NEXT PR |
...
NEXT PR: the core carries the `renderSource` / `shareSource` / `componentPath`
/ `actionSource` slots and the `Sources` seam; the Rails locators that fill them
are the next PR's, so every slot is `null` today.

| # | Contract (quoted) | Verdict |
|---|---|---|
| J1 | `renderSource` = first caller frame outside package src and `/vendor/` (L2594-2620) | NEXT PR — MATCH (first frame under `Rails.root`, outside the gem and `vendor/bundle`) |
| J2 | Per-prop `renderSource` line found by regex `['"]key['"]\s*=>` scanning up to 100 lines from the render line (L272-275, L2735-2753) | NEXT PR — MATCH (Ruby regex also takes `key:`) |
| J3 | `shareSource` per shared key, from the `share()` call site or the middleware `share()` method body (L2296-2343, L2627-2654) | NEXT PR — MATCH (`inertia_share` call site or block source) |
| J4 | `route.actionSource` = file/line of the action's reflection (L2760-2784) | NEXT PR — MATCH (`instance_method(action).source_location`) |
| J5 | `componentPath` from the view finder (L2426) | NEXT PR — MATCH (`devtools_component_paths` roots, six extensions) |
