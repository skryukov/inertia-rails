...
running code in [`spec/hosts`](spec/hosts): they are the reference adapters,
held to one set of expectations (`spec/hosts/support/inertia_host_examples.rb`).
They mount real web frameworks, so they run as a pass of their own:
`bundle exec rspec spec/hosts` from `gems/inertia-core`.
