    ...
    class Prop
      ...
      class Options
        ...
        KNOWN = (DELIVERY + MERGING + MERGE_PATHS + ONCE_PARTS + DEFER_PARTS +
                 %i[once live cache value producer]).freeze
        ...
        MODIFIER_PRESETS = { once: 'once: true', merge: 'merge: true', live: 'live: { on:, channel: }' }.freeze
        ...
        def refuse_live_merge!
          return unless on?(:live) && merges?

          raise ArgumentError,
                'Cannot combine `merge` with `live`: a live update replaces the prop, ' \
