...
module Inertia
  module Core
    # The head of one render, keyed by head key: adding a tag with a key the
    # head already holds replaces it, and there is one `<title>`.
    class MetaTagBuilder
      def initialize(tag_class: MetaTag)
        @tag_class = tag_class
        @meta_tags = {}
      end
      ...
      def add_new_tag(new_tag_data)
        new_tag = @tag_class.new(**new_tag_data)
