...
* Refuse known prop options that scroll props do not support (`merge`, `append`, `prepend`, `once`, `always`, `expires_in`, `fresh`, `value`, `cache`), `optional:` combined with `defer:`, and `group:` without `defer:`, instead of silently forwarding them to the scroll metadata adapters where they vanished; genuinely unknown options still reach custom adapters untouched (@skryukov)
